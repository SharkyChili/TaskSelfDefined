...
public class TakeAllQueue<E> extends AbstractQueue<E>
     ...
     */
    public TakeAllQueue(int capacity) {
        this(capacity, false);
    }
     ...
     * @throws IllegalArgumentException if {@code capacity < 1}
     */
    public TakeAllQueue(int capacity, boolean fair) {
        if (capacity <= 0)
    ...
    private class Itr implements Iterator<E> {
        ...
        Itr() {
            ...
            lastRet = NONE;
            final ReentrantLock lock = TakeAllQueue.this.lock;
            lock.lock();
            try {
                if (count == 0) {
                    // assert itrs == null;
                    ...
                    final int takeIndex = TakeAllQueue.this.takeIndex;
